...
class Metrics7Class:
    ...
    All primary metrics aligned with DMD (run_dmd.py MetricsTop._eval_regression):
      - Acc_7: all samples (round then compare)
      - Acc_2: ONLY non-zero label samples (p_cont > 0 vs t > 0)
      - F1_score: ONLY non-zero label samples (manual TP/FP/FN)
      - MAE: ALL samples (continuous regression MAE)
      - Corr: ALL samples (Pearson correlation)
    """
    ...
    def compute(self, logits7, logits2, labels):
        ...
        p_cls = torch.argmax(logits7, dim=1)
        p_cont = p_cls.float() - 3.0
        ...
        # Acc-7: ALL samples
        targets_cls = (labels.long() + 3).clamp(0, 6)
        ...
        # Auxiliary metrics (reference only)
        from sklearn.metrics import f1_score as sklearn_f1
        pred_cls_np = p_cls.cpu().numpy()
        targ_cls_np = targets_cls.cpu().numpy()
        f1_w = sklearn_f1(targ_cls_np, pred_cls_np, average='weighted', zero_division=0)
        mae_class = torch.abs(p_cls.float() - targets_cls.float()).mean().item()
        
        targets2_all = (labels > 0).long().cpu().numpy()
        pred2_np = preds2.cpu().numpy()
        preds2_from7 = (p_cls >= 4).long().cpu().numpy()
        acc2_from7 = float(np.mean(preds2_from7 == targets2_all))
        acc2_direct = float(np.mean(pred2_np == targets2_all))
        
        return {
            "Acc_7": round(float(acc7), 4),
            "Acc_2": round(float(acc2), 4),
            ...
            "MAE": round(float(mae), 4),
            "MAE_class": round(float(mae_class), 4),
            "Corr": round(corr, 4),
        }
...
class MERMTLTrainer:
    ...
    def do_train(self, model, dataloader, return_epoch_results=False):
        ...
        while True:
            ...
            left = getattr(self.args, 'update_epochs', 1)
            ...
            with tqdm(dataloader['train'], desc=f"Ep {epochs}", ascii=False, 
                ...
                for batch_idx, bd in enumerate(td):
                    if left == getattr(self.args, 'update_epochs', 1):
                        opt.zero_grad()
                    left -= 1
