...
def _collect_log_paths(logs_dir, exp_name_patterns):
    ...
    Returns: dict of {mode: {variant_name: [log_paths]}}
    where mode is 'aligned' or 'unaligned'.
    """
    log_files = {'aligned': {}, 'unaligned': {}}
    for root, dirs, files in os.walk(logs_dir):
        ...
        if 'unaligned' in dir_lower:
            mode = 'unaligned'
        elif 'aligned' in dir_lower:
            mode = 'aligned'
        else:
            mode = 'aligned'  # default fallback

        for pattern, key in exp_name_patterns:
            if pattern.lower() in dir_lower:
                if key not in log_files[mode]:
                    log_files[mode][key] = []
                log_files[mode][key].append(log_path)
                break
    return log_files
...
def plot_training_curves(logs_dir='./logs/mermtl',
                         ...
                         fig_size=(10, 5)):
    """
    Plot loss weight dynamics for MER-MTL-TT and MER-MTL-MP,
    generating separate figures for Aligned and Unaligned conditions.
    ...
    log_files_by_mode = _collect_log_paths(logs_dir, patterns)

    if not log_files_by_mode or not any(log_files_by_mode.get(m) for m in ('aligned', 'unaligned')):
    ...
    for mode in ['aligned', 'unaligned']:
        ...
        if not mode_logs:
            print(f'  WARNING: No logs found for mode={mode}')
            continue

        variants = [(k, v) for k, v in patterns if v in mode_logs]
        if not variants:
            continue

        # Each mode gets its own figure: TT and MP stacked vertically
        n_variants = len(variants)
        fig, axes = plt.subplots(n_variants, 1,
                                 figsize=(6, 3.5 * n_variants),
        ...
        for ax, (pat, variant) in zip(axes, variants):
            ...
            ax.set_ylabel('Effective Loss Weight', fontsize=10)
            ax.set_title(variant, fontsize=11, fontweight='bold')
            ax.legend(fontsize=8, loc='lower right')
            ax.grid(True, alpha=0.3, linestyle='--')

        fig.suptitle(f'Adaptive Loss Weight Dynamics ({mode.capitalize()})',
                     fontsize=12, fontweight='bold', y=1.02)
        plt.tight_layout()

        path_png = os.path.join(output_dir, f'fig_training_curves_{mode}.png')
        path_pdf = os.path.join(output_dir, f'fig_training_curves_{mode}.pdf')
        fig.savefig(path_png, dpi=300, bbox_inches='tight')
        fig.savefig(path_pdf, dpi=300, bbox_inches='tight')
        plt.close(fig)
        print(f'  Saved: {path_png}')
        print(f'  Saved: {path_pdf}')
        saved_paths[mode] = path_pdf
